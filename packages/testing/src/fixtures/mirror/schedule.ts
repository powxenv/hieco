import type { Schedule, EntityId } from "@hieco/mirror";
...
const createSchedule = (options: ScheduleFixtureOptions = {}): Schedule => {
  const consensusTimestamp =
    options.consensus_timestamp ?? options.timestamp ?? Date.now().toString();

  return {
    admin_key: options.admin_key ?? null,
    consensus_timestamp: consensusTimestamp,
    creator_account_id: options.creator_account_id ?? ("0.0.1" as EntityId),
    deleted: options.deleted ?? false,
    executed_timestamp: options.executed_timestamp ?? null,
